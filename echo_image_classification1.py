...
class echo_image_classify1:
    ...
    def home_window(self):
        ...
        canvas.create_rectangle(0, 60, w, h, fill=self.body_color)
        ...
        text_title = canvas.create_text(450, 100, text="SELECT IMAGE(ECHO)", font=("Times New Roman", 20), fill=self.text_color)
        blink_text()
        ...
        def training():
            def write_dataset(disease,value,Stage,treatment,mortality):
                file = 'csv_dataset/training-dataset_echo.csv'
                pr_chk = 0
                with open(file) as f:
                    ...
                    for row in reader:
                        ...
                        if int(t1) == int(value):
                            pr_chk += 1
                if pr_chk == 0:
                    file1 = 'csv_dataset/training-dataset_echo1.csv'
                    with open(file) as f, open('csv_dataset/training-dataset_echo1.csv', 'w', encoding='utf-8', newline='') as csvfile:
                        reader = csv.DictReader(f, delimiter=',')
                        ...
                        csvfile.close()
                        shutil.copy('csv_dataset/training-dataset_echo1.csv', file)
                    ...
                    model.save("echo/cnn_rnn_model.keras")
                    np.save("echo/label_encoder_classes.npy", le.classes_)
                    joblib.dump(scaler, "echo/scaler.save")
                    return "success"
                else:
            ...
            def exit_program():
                ...
                if (disease == ""):
                    ...
                    msg = write_dataset(disease=disease, value=echo_image_classify1.f_value,Stage=Stage,treatment=treatment,mortality=mortality)
        ...
        def testing():
            ...
            nn = FE([2, 2, 1])
            nn.glcm_extract(z)
            echo_image_classify1.f_value = nn.result()
            ################################################
            ...
            treatment='-'
            mortality='-'
            file1 = 'csv_dataset/training-dataset_echo.csv'
            with open(file1) as f:
                reader = csv.DictReader(f, delimiter=',')
                for row in reader:
                    t1 = row['value']
                    t2 = row['label']
                    if int(echo_image_classify1.f_value) == int(t1):
                        result=t2
                        stage = row['stage']
            ...
            echo_image_classify1.e1.append(etime)
            index=echo_image_classify1.f_value
            ####################################

            csv_file = "training-dataset.csv"  # Replace with your CSV file name
            ############################# classification
            model = load_model("echo/cnn_rnn_model.keras")
            scaler = joblib.load("echo/scaler.save")
            label_classes = np.load("echo/label_encoder_classes.npy", allow_pickle=True)
            df = pd.read_csv("csv_dataset/training-dataset_echo.csv")
            ...
            features.append(float(echo_image_classify1.f_value))
            ...
            echo_image_classify1.e1.append(duration_time)
            echo_image_classify1.e1.sort(reverse=True)
            ...
            data = [echo_image_classify1.e1[0], echo_image_classify1.e1[1]]
            lables = ['Existing', 'Proposed']
        ...
        canvas.create_window(280, 500, window=b1)
        ...
        canvas.create_window(w - 280, 500, window=b9)
